// app/api/gtfs/upload/route.js - USES STANDARDIZED AUTH FROM lib/auth.js
...
import { authenticateUser, checkProjectAccess } from "@/lib/auth" // STANDARDIZED AUTH
...
/**
 * STANDARDIZED: Helper function to create a new project
...
async function createAutoProject(userId, fileName) {
  ...
  const newProject = await prisma.userProject.create({
    data: {
      ...
      owner_id: userId, // Use verified userId
      is_active: true,
    },
    select: {
      id: true,
...
/**
 * STANDARDIZED: Helper function to verify project access or create one
 * @param {string} userId - User ID from authenticated token
 * @param {string} projectId - Project ID (optional)
 * @param {string} fileName - File name for auto-created project
 * @returns {Promise<object>} - Project object
 */
async function verifyOrCreateProject(userId, projectId, fileName) {
  // If no projectId provided, create a new one
  if (!projectId) {
    return await createAutoProject(userId, fileName || "GTFS-Data")
  }

  // STANDARDIZED: Use checkProjectAccess from lib/auth.js
  const accessResult = await checkProjectAccess(userId, projectId, "EDITOR")

  // If project not found or no access, create a new one
  if (!accessResult.hasAccess) {
    return await createAutoProject(userId, fileName || "GTFS-Data")
  }

  return accessResult.project
}
...
class GTFSUploadProcessor {
  ...
  generateLogFile() {
    const logTime = new Date()
      ...
      .replace(/[-:]/g, "")
      .replace(/\..+/, "")
    return join(CONFIG.LOG_DIR, `gtfs_debug_${this.userId}_${logTime}.log`)
  }
  ...
  normalizeRecord(model, record) {
    ...
    const normalizers = {
      stop: () => {
        // FIX: Sanitize and validate coordinate formatting
        let lat = normalized.stop_lat
        let lon = normalized.stop_lon

        // 1. Replace commas with periods to handle locale differences
        ...
        // 2. Convert to float after sanitizing
  ...
  async loadExistingIds() {
    const queries = [
      {
        key: "agencyIds",
        sql: `SELECT agency_id FROM "Agency" WHERE project_id = $1`,
      },
      {
        key: "stopIds",
        sql: `SELECT stop_id FROM "Stop" WHERE project_id = $1`,
      },
      {
        key: "routeIds",
        sql: `SELECT route_id FROM "Route" WHERE project_id = $1`,
      },
      {
        key: "tripIds",
        sql: `SELECT trip_id FROM "Trip" WHERE project_id = $1`,
      },
      {
        key: "serviceIds",
        sql: `SELECT service_id FROM "Calendar" WHERE project_id = $1 UNION SELECT service_id FROM "CalendarDate" WHERE project_id = $1`,
      },
    ]
    ...
    for (const { key, sql } of queries) {
      try {
        ...
        await this.logToFile(
          `🔍 Loaded ${ids.size} existing ${key} for project ${this.projectId}.`
        )
      } catch (error) {
        await this.logToFile(
          `⚠️ Failed to load existing ${key}: ${error.message}`
        )
  ...
  filterRecordsByDependencies(model, records) {
    ...
    if (filtered.length < records.length) {
      this.logToFile(
        ...
        } records filtered due to missing dependencies.`
      )
    }
    return filtered
  }
  ...
  async processStopsHierarchically(records) {
    ...
    try {
      // --- STEP 1: Insert all independent stops (those without a parent) ---
      if (stopsWithoutParent.length > 0) {
        await this.logToFile(
          `\n--- Inserting ${stopsWithoutParent.length} stops without parents... ---`
        )
        const insertedCount = await this.insertChunked(
          "stop",
          "stop_id",
          stopsWithoutParent
        )
        ...
        await this.logToFile(
          `--- Successfully inserted ${insertedCount} independent stops. ---`
        )
      }

      // --- STEP 2: Insert all dependent stops (those with a parent) ---
      if (stopsWithParent.length > 0) {
        ...
        totalInserted += insertedCount
        await this.logToFile(
          `--- Successfully inserted ${insertedCount} dependent stops. ---`
        )
      }
    } catch (error) {
      await this.logToFile(
        `❌ A critical error occurred during hierarchical stop processing.`
      )

      // --- DEBUG LOGIC TO FIND THE EXACT BAD RECORD ---
      const allParentIdsInDb = this.referenceIds.get("stopIds")
      const missingParents = stopsWithParent.filter(
        (child) => !allParentIdsInDb.has(child.parent_station)
      )

      if (missingParents.length > 0) {
        const problemChild = missingParents[0]
        console.log(
          "\n\n======================================================="
        )
        console.log("💥 DEBUG: Found a child stop with a missing parent!")
        console.log(`Child Stop ID:      '${problemChild.stop_id}'`)
        console.log(`References Parent ID: '${problemChild.parent_station}'`)
        console.log(
          "This parent ID was NOT found among the successfully inserted stops."
        )
        console.log(
          "Check for hidden spaces, case-sensitivity issues, or typos in your stops.txt file."
        )
        console.log(
          "=======================================================\n\n"
        )
  ...
  async insertChunked(model, primaryKey, records) {
    ...
    const chunks = Array.from(
      ...
      (_, i) =>
        records.slice(
          i * CONFIG.CHUNK_SIZE,
          i * CONFIG.CHUNK_SIZE + CONFIG.CHUNK_SIZE
        )
    )
    ...
    for (const chunk of chunks) {
      try {
        ...
        // This logic correctly prevents duplicate primary keys from being sent
        if (primaryKey) {
          const uniqueRecords = new Map()
          chunk.forEach((record) => {
            if (record[primaryKey]) {
              uniqueRecords.set(record[primaryKey], record)
            }
          })
        ...
        if (primaryKey) {
          const chunkIds = finalChunk.map((r) => r[primaryKey]).filter(Boolean)
          if (!this.loggedIds[model]) this.loggedIds[model] = []
          if (this.loggedIds[model].length < CONFIG.MAX_LOG_IDS) {
            this.loggedIds[model].push(...chunkIds)
          }
        }
      } catch (error) {
        console.log(error, "<<<<<<<<<<<<<<<<")
        const errorMessage =
          error?.meta?.cause || error.message || "Unknown error"
        await this.logToFile(
          `❌ Failed to insert ${model} chunk: ${errorMessage}`
        )
      }
    }
    return totalInserted
  ...
  async processTable(tableName, zip) {
    ...
    try {
      ...
      const normalizedRecords = rawRecords.map((record) =>
        this.normalizeRecord(tableConfig.model, record)
      )
      const filteredRecords = this.filterRecordsByDependencies(
        tableConfig.model,
        normalizedRecords
      )
      ...
      if (!filteredRecords.length) {
        await this.logToFile(
          `- ${tableName}: No valid records after dependency filtering, skipping.`
        )
        return
      }
      ...
      if (tableConfig.model === "stop") {
        ...
        totalInserted = await this.insertChunked(
          tableConfig.model,
      ...
      await this.logToFile(
        `✅ ${tableConfig.model}: ${totalInserted} records processed.`
      )
  ...
  async process(zipBuffer) {
    await this.logToFile(
      `🚀 Starting GTFS upload process for user ${this.userId}, project ${this.projectId}`
    )
    try {
...
export async function POST(request) {
  try {
    // STANDARDIZED: Use authenticateUser from lib/auth.js
    ...
    // STANDARDIZED: Use user.id from authenticated user
    const project = await verifyOrCreateProject(user.id, projectId, file.name)
    projectId = project.id

    const buffer = Buffer.from(await file.arrayBuffer())
    const processor = new GTFSUploadProcessor(user.id, projectId)
    ...
    return NextResponse.json({
      ...
      project: {
        ...
        created: !formData.get("project_id"), // Indicate if project was auto-created
      },
      ...result,
    })
  } catch (error) {
    ...
    return NextResponse.json(
      { error: error.message || "Error processing GTFS file" },
      { status: 500 }
    )
  }
}
